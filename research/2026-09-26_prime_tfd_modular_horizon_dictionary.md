...
## 10. The causal Cayley radius is an entanglement monotone

For the pure two-mode squeezed prime state, the logarithmic negativity is

\[
E_{\mathcal N,p}=2\kappa_p.
\]

Since the previously derived causal/impedance radius is
...
we get the exact quantum-information identity

\[
\boxed{
r_p=e^{-E_{\mathcal N,p}},
\qquad
E_{\mathcal N,p}
=
\log\frac{\sqrt p+1}{\sqrt p-1}.
}
\]

So the prime-local Cayley contraction is literally the exponential of minus the
TFD logarithmic negativity.

The reduced-state purity is also exact:
...
With \(p^{-1/2}=\tanh\kappa_p\),

\[
\boxed{
\operatorname{Tr}\rho_p^2
=
\operatorname{sech}(2\kappa_p)
=
\operatorname{sech}(E_{\mathcal N,p}).
}
\]

Thus the local arithmetic transfer variables have a direct entanglement meaning:
- \(\kappa_p\): squeezing / half logarithmic negativity;
- \(r_p\): exponentiated negative logarithmic negativity;
- \(\mu_p=2\sinh\kappa_p\): Casimir mass generated by the same entanglement parameter.
...
\mu_p
=
2\sinh(E_{\mathcal N,p}/2),
\qquad
E_{\mathcal N,p}
=
2\,\operatorname{arsinh}(\mu_p/2).
}
\]
...
So the golden point is simultaneously the unit-Casimir-mass prime channel and the
prime TFD whose logarithmic negativity is \(2\log\varphi\).

## 11. p-adic radial depth is the entanglement-energy ladder

The local occupation label \(a=v_p(x)\) is also the valuation-shell / Bruhat--Tits
radial-depth coordinate used in the p-adic conformal program.

The modular entanglement energy of level \(a\) is

\[
\boxed{
\varepsilon_{p,a}=a\log p.
}
\]

Meanwhile the TFD amplitude is
...
So the p-adic radial depth and the thermal entanglement spectrum are the same discrete
ladder in two languages:
...
\longleftrightarrow
\text{modular entanglement energy}.
}
\]

The amplitude \(p^{-a/2}\) is exactly a half-density radial weight. Squaring it gives
the Haar/Gibbs probability \(p^{-a}\).

This gives a concrete local mechanism by which p-adic holographic radial geometry and
arithmetic thermofield entanglement can be two representations of the same finite-place
state.
