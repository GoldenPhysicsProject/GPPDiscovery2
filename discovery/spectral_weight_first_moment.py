...
def main():
    ...
    # Independent high-precision finite partial sum. Keeping this numerical
    # avoids pathological growth of exact rational denominators in a long CAS sum.
    partial = mp.fsum(1 / mp.mpf(2 * j + 1) ** 3 for j in range(20000))
    partial_target = (1 - mp.mpf(2) ** -3) * mp.zeta(3)
    partial_err = abs(partial - partial_target)
    ...
    print("odd partial err  =", mp.nstr(partial_err, 8))

    assert err < mp.mpf("1e-65")
    assert sp.simplify(symbolic - expected) == 0
    assert partial_err < mp.mpf("4e-10")
