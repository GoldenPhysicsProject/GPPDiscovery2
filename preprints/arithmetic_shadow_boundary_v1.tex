...
\section{The canonical global defect colligation}

The completed boundary quotient is unimodular on the real axis whether or not it is
causal.  Thus multiplication by
\[
\Theta_\omega(t)=\frac{\xi(a+it)}{\xi(a-it)}
\]
is a unitary operator \(U_\omega\) on boundary \(L^2\).  Compress it to the causal Hardy
space:
\[
T_\omega
=
...
Hence \(T_\omega\) is a contraction and its incoming defect is
\[
D_{\omega,\mathrm{in}}
=
(I-T_\omega^*T_\omega)^{1/2}.
\]
The exact identity above gives
\[
D_{\omega,\mathrm{in}}^2
=
\mathfrak H_\omega^*\mathfrak H_\omega.
\]
Therefore
...
\mathrm{RH}
\iff
D_{\omega,\mathrm{in}}=0
\quad\text{for every }\omega>0.
}
\]

The corresponding first-order skew operator
\[
\mathcal Q_\omega
=
\begin{pmatrix}
...
satisfies
\[
\mathcal Q_\omega^*=-\mathcal Q_\omega,
\qquad
-\mathcal Q_\omega^2
=
\begin{pmatrix}
...
\[
\boxed{
I+\mathcal Q_\omega^2
=
D_{\omega,\mathrm{in}}^2.
}
\]
Thus the failure of the global first-order operator to be an exact complex structure is
precisely the positive incoming leakage energy.

Every contraction has a canonical Julia--Halmos unitary colligation
\[
\mathcal U(T)=
\begin{pmatrix}
T&D_{T^*}\\
...
D_{T^*}=(I-TT^*)^{1/2}.
\]
Applied to \(T_\omega\), this is a zero-independent global multichannel unitary
colligation constructed directly from the arithmetic boundary function \(\xi\).

The local prime beam splitter
\[
S_p=
\begin{pmatrix}
a_p&\sqrt{1-a_p^2}\\
...
\end{pmatrix}
\]
is exactly the scalar Julia colligation of the contraction \(a_p=p^{-1/2}\).  The local
Euler shadow channel and the global Hardy boundary channel are therefore the scalar and
operator-valued instances of the same construction.

There are, however, two different defects.  If the boundary function has a coprime inner
quotient factorization
\[
\Theta_\omega=I_\omega\overline{B_\omega},
\]
then Toeplitz ordering gives
\[
T_\omega=T_{B_\omega}^*T_{I_\omega}
\]
and consequently
\[
\boxed{
I-T_\omega^*T_\omega
=
T_{I_\omega}^*P_{K_{B_\omega}}T_{I_\omega}.
}
\]
Thus the incoming defect is a positive pullback of the bad-zero model space
\[
K_{B_\omega}=H^2_+\ominus B_\omega H^2_+.
\]
It vanishes precisely when the bad denominator is absent.

By contrast, the outgoing defect
\[
D_{\omega,\mathrm{out}}^2
=
I-T_\omega T_\omega^*
\]
may remain nonzero when \(T_\omega\) is an isometry.  Under RH this is the familiar
model-space defect associated with the ordinary critical-line inner factor.  The physical
requirement is therefore not two-sided unitarity of the causal transfer.  It is
\emph{incoming isometry}.  The outgoing defect carries spectral information that should
not be discarded.
