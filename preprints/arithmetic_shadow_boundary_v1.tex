...
\begin{theorem}[Conservative completion]
Suppose
\[
...
\[
\boxed{
\lambda=\varphi^2,\qquad c=\varphi.
}
\]
Consequently the finite-place parameter is uniquely \(q=5\).
\end{theorem}

\begin{proof}
Since
\[
|1-e^{i\theta}|^2+1=3-2\cos\theta
\]
and
\[
|\lambda-e^{i\theta}|^2
...
\]
equality for all \(\theta\) forces equality of the constant and cosine coefficients:
\[
3c^2=\lambda^2+1,\qquad c^2=\lambda.
\]
Eliminating \(c\) gives the quadratic equation.
\end{proof}
...
|\alpha|^2+|\beta|^2=1
\]
is simply the boundary-symbol form of
\[
A_{\log\varphi}^*A_{\log\varphi}=L+I.
\]

This is one of the central identifications of the paper: the normalized Hardy pair, the unit-mass Casimir factor, and the \(q=5\) finite-place shadow center are one transfer system written in three coordinate languages.
...
\item the pair
\[
\alpha(z)=\sqrt{\lambda}\frac{1-z}{\lambda-z},
\qquad
\beta(z)=\frac{\sqrt{\lambda}}{\lambda-z}
\]
is conservative on \(\T\).
\end{enumerate}
\end{theorem}
...
\]
hence the trace-three polynomial.  Finally the conservative condition is equivalent, by the preceding theorem, to
\[
\lambda^2-3\lambda+1=0.
\]
